...
function init() {
	...
	settingsPromise.then(() => {
		...
		return loadLocaleMessages(lang).then(() => {
			...
			// move from localStorage to indexedDB Sep. 29 2016
			if (localStorage.length > 3) {
				for (var key in localStorage) {
					console.log("transfer: " + key);
					if (key == "lsid" || key == "newestMail") {
						// ignore it
					} else if (localStorage[key] === "true" || localStorage[key] === "false") {
						Settings.store(key, JSON.parse(localStorage[key]));
					} else if (localStorage[key].length >= 10 && !isNaN(new Date(localStorage[key]).getTime())) {
						Settings.store(key, new Date(localStorage[key]));
					} else {
						Settings.store(key, localStorage[key]);
					}
				}
				...
				function convertToObject(key) {
					try {
						if (localStorage[key] != null) {
							Settings.store(key, JSON.parse(localStorage[key], dateReviver));
						}
					} catch (e) {
				...
				convertToObject("_newestMail");
				convertToObject("autoSave");
				convertToObject("tokenResponsesContacts");
				convertToObject("tokenResponsesEmails");
				convertToObject("tokenResponsesPeople");
				convertToObject("tokenResponsesProfiles");
			...
			if (chrome.alarms) {
				chrome.alarms.onAlarm.addListener(function(alarm) {
					if (alarm.name == "extensionUpdatedSync") {
						...
						if (skinsIds.length) {
							Controller.getSkins(skinsIds, Settings.read("_lastUpdateSkins")).then(function(skins) {
			...
			*/
			...
			pollAccounts({showNotification:true, source:Source.STARTUP, refresh:true}).then(function() {
