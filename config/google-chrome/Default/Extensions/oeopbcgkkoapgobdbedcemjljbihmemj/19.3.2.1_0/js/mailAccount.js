...
function MailAccount(accountParams) {
 					   ...
 					   complete: function(jqXHR, textStatus) {
 						   if (textStatus == "success") {
 							   try {
 								   ...
 								   var $responseText = parseHtmlToJQuery(jqXHR.responseText);
   ...
   this.syncSignInId = function(secondCall) {
	   ...
	   return new Promise(function(resolve, reject) {
		   $.ajax(MAIL_DOMAIN_AND_PATH + "u/?authuser=" + encodeURIComponent(mailAddress)).done((data, textStatus, jqXHR) => {
			   ...
			   complete: function(jqXHR, textStatus) {
				   ...
				   if (textStatus == "success") {
					   ...
					   if (testGmailQuestion || (jqXHR.responseURL && jqXHR.responseURL.indexOf("v=lui") != -1)) {
						   ...
						   var $responseWrapper = parseHtmlToJQuery(responseText);
   ...
   this.hasHiddenTabs = function() {
		var gmailSettings = that.getSetting("gmailSettings");
		return that.isUsingGmailCategories() && gmailSettings.tabs.length <= 4; // 5 means ok because they are all shown
   }
