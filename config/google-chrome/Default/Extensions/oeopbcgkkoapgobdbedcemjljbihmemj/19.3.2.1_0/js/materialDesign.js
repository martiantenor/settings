...
function isVisibleInScrollArea($node, $scroll, mailIndex) {
    ...
    console.info($scroll.is(":visible") + " y: " + y + " " + "y:" + y + " vph: " + vpH);
...
function init() {
	...
	return bgObjectsReady.then(() => {
		...
		$("#maximize").mousedown(function(e) {
			if (isCtrlPressed(e)) {
				//bg.openInPopup();
				openWindowInCenter("materialDesign.html", '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes', Settings.read("popupWidth"), Settings.read("popupHeight"));
		...
		// FYI whole bloack below used to be above document.ready
		return polymerPromise.then(() => {
			...
			return bgObjectsReady.then(() => {
				...
				if (shouldShowReducedDonationMsg(true)) {
					...
					;
				} else if (!Settings.read("tryMyOtherExtensionsClicked") && daysElapsedSinceFirstInstalled() > 3) { // previous prefs: writeAboutMeClicked, tryMyOtherExtensionsClicked
					/*
						...
						if (!installed) {
							...
							$("#newsNotification")
								...
								.click(function() {
									...
									openUrl("https://jasonsavard.com/checkerPlusForGmail?ref=calpopup2");
								})
							;
						}
					});
					...
					if (Settings.read("accountAddingMethod") == "autoDetect" && !mustUseAddAccount) {
						openDialog($dialog).then(function(response) {
							if (response == "ok") {
								...
								$dialog[0].close();
								refresh().then(() => {
									if (getAccountsWithErrors(accounts).length) {
										location.reload();
									}
								});
							} else {
